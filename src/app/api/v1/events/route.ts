...
export const POST = async (req: NextRequest) => {
  try {
    ...
    if (!authHeader) {
      return NextResponse.json({ message: "Unaouthorized" }, { status: 401 })
    }

    if (!authHeader.startsWith("Bearer")) {
      return NextResponse.json(
        {
          message: "Invalid auth header format. Exptected: 'Bearer [API_KEY]'",
        },
        { status: 401 }
      )
    }
    ...
    if (!category) {
      return NextResponse.json(
        {
          message: `You dont have a category named ${validationResult}`,
        },
        { status: 404 }
      )
    }
